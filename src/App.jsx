...
// Allium API helper - routes through /api/allium proxy to avoid CORS
async function alliumFetch(endpoint, body) {
  // Try serverless proxy first (works on Vercel and avoids CORS)
  try {
    const proxyRes = await fetch(`/api/allium?endpoint=${encodeURIComponent(endpoint)}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(body),
    });
    if (proxyRes.ok) return await proxyRes.json();
    // If proxy returns error, try direct
  } catch (e) { /* proxy not available */ }
  
  // Fallback: direct call (may fail due to CORS in browser)
  const res = await fetch(`https://api.allium.so${endpoint}`, {
    method: "POST",
...
export default function AlliumIntelApp() {
  ...
  const fetchLivePrices = useCallback(async () => {
    try {
      const json = await alliumFetch("/api/v1/developer/prices", [
    ...
    <div style={{ minHeight: "100vh", background: theme.bg, color: theme.text, fontFamily: "'Inter','Helvetica Neue',sans-serif", transition: "background 0.6s ease, color 0.4s ease" }}>
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 1000, margin: "0 auto", padding: "36px 24px" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 32, flexWrap: "wrap", gap: 16 }}>
          <div>
            ...
            <p style={{ fontSize: 12, color: theme.textDim }}>
              Real-time market intelligence & on-chain analytics · Powered by Allium MCP
            </p>
          </div>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 28, flexWrap: "wrap" }}>
          <TabButton label="🐋 Whale Watch" active={tab === "whales"} onClick={() => setTab("whales")} color="#00E39E" />
